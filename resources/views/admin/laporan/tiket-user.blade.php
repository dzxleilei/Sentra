                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($groupedTickets as $kodeBooking => $items)
                        ...
                        <tr class="align-top hover:bg-slate-50">
                            <td class="px-6 py-3.5 font-mono text-sm text-slate-700">{{ $kodeBooking }}</td>
                            <td class="px-6 py-3.5 text-sm text-slate-700">{{ $first->tipe }}</td>
                                        ...
                                        <tbody>
                                            @foreach($items as $item)
                                                <tr class="border-b border-slate-100 align-top">
                                                    ...
                                                    <td class="px-4 py-3">
                                                        @if($item->foto_awal)
                                                            <a href="{{ route('admin.laporan.tiket.foto', ['id' => $item->id, 'jenis' => 'awal']) }}" target="_blank" class="font-semibold text-blue-700 hover:text-blue-900">Lihat</a>
                                                        @else
                                                            -
                                                        @endif
                                                    </td>
                                                    <td class="px-4 py-3">
                                                        @if($item->foto_akhir)
                                                            <a href="{{ route('admin.laporan.tiket.foto', ['id' => $item->id, 'jenis' => 'akhir']) }}" target="_blank" class="font-semibold text-blue-700 hover:text-blue-900">Lihat</a>
                                                        @else
                                                            -
                                                        @endif
                                                    </td>
