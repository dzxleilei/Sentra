    ...
    <form method="GET" class="mb-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-4">
        ...
        <div>
            ...
            <select id="sort_by" name="sort_by" class="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                <option value="kode_thing" {{ ($sortBy ?? 'kode_thing') === 'kode_thing' ? 'selected' : '' }}>Kode Barang</option>
                <option value="nama" {{ ($sortBy ?? 'kode_thing') === 'nama' ? 'selected' : '' }}>Nama Barang</option>
                <option value="status" {{ ($sortBy ?? 'kode_thing') === 'status' ? 'selected' : '' }}>Status</option>
            </select>
        </div>
        <div>
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($barang as $item)
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-3.5 text-sm font-mono text-slate-700">{{ $item->kode_thing }}</td>
