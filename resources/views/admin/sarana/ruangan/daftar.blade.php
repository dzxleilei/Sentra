    ...
    <form method="GET" class="mb-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-6">
        ...
        <div>
            ...
            <select id="sort_by" name="sort_by" class="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                <option value="kode_room" {{ ($sortBy ?? 'kode_room') === 'kode_room' ? 'selected' : '' }}>Kode Ruangan</option>
                <option value="nama" {{ ($sortBy ?? 'kode_room') === 'nama' ? 'selected' : '' }}>Nama Ruangan</option>
                <option value="lantai" {{ ($sortBy ?? 'kode_room') === 'lantai' ? 'selected' : '' }}>Lantai</option>
                <option value="status" {{ ($sortBy ?? 'kode_room') === 'status' ? 'selected' : '' }}>Status</option>
            </select>
        </div>
        <div>
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($ruangan as $item)
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-3.5 text-sm font-mono text-slate-700">{{ $item->kode_room }}</td>
                        <td class="px-6 py-3.5 text-sm text-slate-700">{{ $item->nama }}</td>
                        <td class="px-6 py-3.5 text-sm text-slate-600">{{ $item->lantai ?? '-' }}</td>
