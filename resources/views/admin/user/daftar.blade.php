...
<div class="max-w-7xl mx-auto p-2">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.user.tambah', ['scope' => $isPeminjamPage ? 'peminjam' : 'staff']) }}" class="rounded-lg bg-blue-600 px-4 py-2 text-white transition hover:bg-blue-700">
                <i class="fas fa-plus mr-1"></i> Tambah User
            </a>
            <a href="{{ route('admin.user.import', ['scope' => $isPeminjamPage ? 'peminjam' : 'staff']) }}" class="rounded-lg border border-slate-300 px-4 py-2 text-slate-700 transition hover:bg-slate-100">
                    ...
                    @forelse($users as $user)
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-3.5 {{ $isPeminjamPage ? '' : 'whitespace-nowrap' }}">
                                ...
                                @else
                                    <a href="{{ route('admin.user.edit', ['id' => $user->id, 'scope' => 'staff']) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-blue-200 bg-blue-50 text-blue-700 transition hover:bg-blue-100" title="Edit {{ $user->name }}" aria-label="Edit {{ $user->name }}">
                        ...
                        @if($isPeminjamPage)
                            <tr id="borrower-action-row-{{ $user->id }}" class="hidden">
                                    ...
                                    <div class="rounded-2xl border border-slate-200 bg-white p-5">
                                        <div class="flex flex-wrap items-center justify-between gap-3">
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                <a href="{{ route('admin.user.edit', ['id' => $user->id, 'scope' => 'peminjam']) }}" class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-blue-700">
                                                    <i class="fas fa-user-pen mr-1"></i> Edit User
                                                </a>
                                                @if($isBlocked)
                                                    <button type="button" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700" onclick="openStatusModal('{{ route('admin.user.buka-blokir', $user->id) }}', 'Buka blokir peminjam', 'Buka blokir untuk {{ $user->name }}?', 'Buka Blokir', 'bg-emerald-600 hover:bg-emerald-700')">
                                                        <i class="fas fa-unlock mr-1"></i> Buka Blokir
                                                    </button>
                                                @else
                                                    <button type="button" class="rounded-lg bg-amber-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-amber-700" onclick="openStatusModal('{{ route('admin.user.blokir', $user->id) }}', 'Blokir peminjam', 'Blokir {{ $user->name }} sekarang?', 'Blokir', 'bg-amber-600 hover:bg-amber-700')">
                                        ...
                                        <div class="mt-4 grid gap-3 md:grid-cols-2">
                                            <div class="rounded-xl border border-slate-200 bg-white p-4">
                                                ...
                                                <p class="mt-2 text-3xl font-bold text-slate-900">{{ (int) $user->penalty_points }}</p>
                                                <p class="mt-1 text-xs text-slate-500">Batas blokir aktif: {{ $threshold }} poin</p>
                                            </div>
                                            <div class="rounded-xl border border-slate-200 bg-white p-4">
                                                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Perubahan Penalti</p>
                                                <form id="penalty-form-{{ $user->id }}" method="POST" class="mt-3 space-y-2">
                                                    @csrf
                                                    <input type="hidden" name="user_id" value="{{ $user->id }}">
                                                    <select name="penalty_action" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" onchange="setPenaltyFormAction({{ $user->id }})" required>
                                                    ...
                                                    <button id="penalty-submit-{{ $user->id }}" type="submit" class="w-full rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700" disabled>Simpan Penalti</button>
                                                </form>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                                            ...
                                            <div class="mt-3 space-y-2">
                                                @forelse($user->penaltyLogs as $log)
                                                    <div class="rounded-xl border border-slate-200 px-3 py-2">
                                                        <div class="flex flex-wrap items-center justify-between gap-2">
                                                            <div>
                                                                ...
                                                                <p class="text-xs text-slate-500">{{ $log->created_at?->format('d M Y H:i') }} · oleh {{ $log->admin?->name ?? '-' }}</p>
                                                            </div>
                                                            <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $log->action === 'add' ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-800' }}">
                                                                {{ match ($log->action) {
                                                                    'add' => 'Tambah',
                                                                    'subtract' => 'Kurangi',
                                                                    'set' => 'Set',
                                                        ...
                                                        <p class="mt-1 text-xs text-slate-600">{{ $log->reason }}</p>
                                                    </div>
                                                @empty
...
function setPenaltyFormAction(userId) {
    ...
    if (action === 'add') {
        form.action = `{{ url('/admin/user') }}/${userId}/penalty/add`;
    ...
    if (action === 'subtract') {
        form.action = `{{ url('/admin/user') }}/${userId}/penalty/subtract`;
