    ...
    <div class="mx-auto min-h-screen max-w-md bg-white shadow-sm md:max-w-full">
        <header class="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur">
            ...
            <div class="mt-1 flex items-center justify-between">
                <h1 class="text-lg font-bold">@yield('page_title', 'Peminjam')</h1>
                <a href="{{ route('peminjam.pengaturan') }}" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-600">
                    Akun
                </a>
            </div>
            <p class="mt-1 text-xs text-slate-500">{{ Auth::user()->name }}</p>
            <p id="header-live-clock" class="mt-1 text-xs font-semibold text-slate-700" data-server-now="{{ $serverNowIso }}">Waktu server: --:--:--</p>
        </header>
        ...
        <nav class="fixed bottom-0 left-0 right-0 z-30 mx-auto grid max-w-md grid-cols-4 border-t border-slate-200 bg-white md:max-w-full">
            <a href="{{ route('peminjam.dashboard') }}" class="px-2 py-3 text-center text-xs {{ request()->routeIs('peminjam.dashboard') ? 'font-bold text-blue-600' : 'text-slate-500' }}">
                <i class="fas fa-home block text-base"></i>
                Dashboard
            </a>
            <a href="{{ route('peminjam.barang') }}" class="px-2 py-3 text-center text-xs {{ request()->routeIs('peminjam.barang') || request()->routeIs('peminjam.ruangan.barang') ? 'font-bold text-blue-600' : 'text-slate-500' }}">
                <i class="fas fa-box block text-base"></i>
                Barang
            </a>
            <a href="{{ route('peminjam.ruangan') }}" class="px-2 py-3 text-center text-xs {{ request()->routeIs('peminjam.ruangan') ? 'font-bold text-blue-600' : 'text-slate-500' }}">
                <i class="fas fa-door-open block text-base"></i>
                Ruangan
            </a>
            <a href="{{ route('peminjam.riwayat') }}" class="px-2 py-3 text-center text-xs {{ request()->routeIs('peminjam.riwayat') || request()->routeIs('peminjam.penalti') || request()->routeIs('peminjam.tiket') ? 'font-bold text-blue-600' : 'text-slate-500' }}">
