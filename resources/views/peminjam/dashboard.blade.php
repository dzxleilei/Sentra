...
@section('content')
    ...
    <section class="grid gap-3">
        <a href="{{ route('peminjam.riwayat') }}" class="group rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 p-4 text-white transition hover:-translate-y-0.5 hover:shadow-lg">
    ...
    <div x-data="{ openDamageModal: false }">
        <section class="mt-5 grid grid-cols-2 gap-3">
            <article class="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <form id="quick-qr-form" action="{{ route('quick-borrow-qr') }}" method="POST" class="mt-4 w-full">
                    @csrf
                    <input id="quick-qr-code" type="hidden" name="qr_code" value="">
                    <button id="open-quick-qr-scanner" type="button" class="w-full rounded-xl bg-blue-600 px-2 py-2 text-xs font-semibold text-white disabled:opacity-60 sm:text-sm" {{ $bookingBlocked ? 'disabled' : '' }}>
