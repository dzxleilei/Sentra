...
@section('title', 'Tiket Booking')
@section('page_title', 'Tiket Booking')
...
@section('content')
    ...
    <article class="rounded-2xl border border-slate-200 p-4">
        ...
        <p class="mt-1 font-mono text-lg font-bold">{{ $borrow->kode_booking }}</p>
        ...
        <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
                ...
                <p class="font-semibold">{{ $ticketItems->count() }} item</p>
            </div>
            <div class="col-span-2">
                <p class="text-xs text-slate-500">Jadwal</p>
                <p class="font-semibold">{{ optional($firstStart)->format('d M Y H:i') }} - {{ optional($firstEnd)->format('H:i') }}</p>
            </div>
            <div class="col-span-2">
                <p class="text-xs text-slate-500">Alasan</p>
                <p class="font-semibold">{{ $borrow->alasan_peminjaman }}@if($borrow->alasan_peminjaman === 'Lainnya' && $borrow->alasan_lainnya) - {{ $borrow->alasan_lainnya }}@endif</p>
            </div>
        </div>
    </article>
    ...
    @if($allBookingItems)
        <section class="mt-4 rounded-2xl border border-slate-200 p-4">
            ...
            <p id="cancel-countdown" data-target="{{ optional($cancelDeadline)->toIso8601String() }}" class="mt-2 text-xs font-semibold text-amber-700"></p>
            <form action="{{ route('booking.cancel') }}" method="POST" class="mt-3">
                @csrf
                <input type="hidden" name="kode_booking" value="{{ $borrow->kode_booking }}">
                <button id="cancel-ticket-btn" type="submit" class="w-full rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60" {{ $canCancelNow ? '' : 'disabled' }}>Batalkan Tiket</button>
            </form>
        </section>
    @endif
    ...
    @if($hasBookingItems)
        <section class="mt-4 rounded-2xl border border-slate-200 p-4">
            ...
            <p class="mt-1 text-xs text-slate-500">
                ...
                @endif
                Check-in paling cepat jam {{ $earliestCheckin }}.
            </p>
            <p id="checkin-countdown" data-target="{{ optional($checkinStartAt)->toIso8601String() }}" class="mt-2 text-xs font-semibold text-blue-700"></p>
            <p id="checkin-deadline-countdown" data-target="{{ optional($checkinDeadline)->toIso8601String() }}" class="mt-1 hidden text-xs font-semibold text-rose-700"></p>
            @if($isItemTicket)
                <p class="mt-2 text-xs text-slate-500">Semua barang wajib discan dari daftar barang di atas sebelum proses check-in.</p>
                <p id="checkin-scan-progress" class="mt-2 text-xs font-semibold text-slate-700">Progress scan: 0 / {{ $bookingTicketItems->count() }}</p>
            @endif
        </section>
    @endif
                ...
                <div class="rounded-xl border border-slate-200 p-3" x-data="{ openReport: false }">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <p class="text-xs text-slate-500">ID Barang: {{ $item->thing->kode_thing ?? '-' }}</p>
                            <p class="mt-1 text-sm font-semibold text-slate-800">{{ $item->thing->nama ?? '-' }}</p>
                            <div class="mt-2 flex flex-wrap gap-2">
                                <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $item->status === 'Selesai' ? 'bg-emerald-100 text-emerald-800' : ($item->status === 'Berlangsung' ? 'bg-blue-100 text-blue-800' : ($item->status === 'Dibatalkan' ? 'bg-slate-200 text-slate-700' : 'bg-amber-100 text-amber-800')) }}">{{ $item->status }}</span>
                                @if($item->status !== 'Selesai')
                                    @if($item->status_checkin)
                                        <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $checkinBadge[$item->status_checkin] ?? 'bg-slate-100 text-slate-700' }}">{{ $item->status_checkin }}</span>
                                    @endif
                                    @if($item->status_checkout)
                                        <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $checkoutBadge[$item->status_checkout] ?? 'bg-slate-100 text-slate-700' }}">{{ $item->status_checkout }}</span>
                                    @endif
                                @endif
                            </div>
                        </div>

                        @if(($isItemTicket && $item->status === 'Booking') || $item->waktu_checkin)
                            <div class="mt-0.5 flex shrink-0 flex-col items-end gap-2">
                                @if($isItemTicket && $item->status === 'Booking')
                                    <div class="flex items-center gap-2">
                                        <span id="scan-state-checkin-{{ $item->thing->kode_thing }}" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-rose-100 text-rose-700" title="Belum discan">
                                        ...
                                        <button
                                            ...
                                            data-mode="checkin"
                                            data-scan-code="{{ strtoupper((string) ($item->thing->kode_thing ?? '')) }}"
                                            data-label="{{ $item->thing->nama ?? 'Barang' }}"
                                            {{ $canCheckinNow ? '' : 'disabled' }}
                                        >
                                            Scan QR
                                        </button>
                                    </div>
    ...
    @if($hasBookingItems)
        <section class="mt-4 rounded-2xl border border-slate-200 p-4">
            ...
            <form action="{{ route('borrow.checkin') }}" method="POST" enctype="multipart/form-data" class="mt-3 space-y-2">
                @csrf
                <input type="hidden" name="kode_booking" value="{{ $borrow->kode_booking }}">
                @if($isItemTicket)
                ...
                @else
                    ...
                    <div class="rounded-xl border border-slate-200 p-3">
                        ...
                        <button type="button" id="start-checkin-scan" class="mt-2 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-60" {{ $canCheckinNow ? '' : 'disabled' }}>Buka Kamera</button>
                    </div>
                @endif
                <div>
                ...
                <button id="checkin-submit-btn" type="submit" class="w-full rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60" {{ $canCheckinNow ? '' : 'disabled' }}>Proses Check-in</button>
            </form>
        </section>
    @endif
        ...
        <section class="mt-4 rounded-2xl border border-slate-200 p-4">
            ...
            <p class="mt-1 text-xs text-slate-500">
                ...
                @else
                    Scan QR ruangan sesuai kode fisik ruangan.
                @endif
                Check-out paling lambat jam {{ $latestCheckout }}.
            </p>
            <p id="checkout-deadline-countdown" data-target="{{ optional($checkoutDeadline)->toIso8601String() }}" class="mt-1 hidden text-xs font-semibold text-rose-700"></p>
            <form action="{{ route('borrow.checkout') }}" method="POST" enctype="multipart/form-data" class="mt-3 space-y-2">
                @csrf
                <input type="hidden" name="kode_booking" value="{{ $borrow->kode_booking }}">
                @if($isItemTicket)
                    ...
                    <div class="rounded-xl border border-slate-200 p-3">
                        ...
                        <p id="checkout-scan-progress" class="mt-2 text-xs font-semibold text-slate-700">Progress scan: 0 / {{ $runningTicketItems->count() }}</p>
                        <div class="mt-3 space-y-2">
                            @foreach($runningTicketItems as $item)
                                <div class="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2">
                                    <div>
                                        <p class="text-sm font-semibold text-slate-800">{{ $item->thing->nama ?? '-' }}</p>
                                        <p class="text-xs text-slate-500">ID Barang: {{ $item->thing->kode_thing ?? '-' }}</p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span id="scan-state-checkout-{{ $item->thing->kode_thing }}" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-rose-100 text-rose-700" title="Belum discan">
                                            <i class="fas fa-xmark"></i>
                                        </span>
                                        <button
                                            type="button"
