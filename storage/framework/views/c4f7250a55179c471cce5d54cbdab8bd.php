...
<?php $__env->startSection('page_title', isset($ruangan) ? 'Edit Ruangan & Isi' : 'Tambah Ruangan & Isi'); ?>
<?php $__env->startSection('page_subtitle', 'Kelola ruangan sekaligus daftar barang di dalamnya'); ?>
...
<div class="max-w-7xl p-2">
    <div class="mb-3 flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <a href="<?php echo e(route('admin.ruangan')); ?>" class="hover:text-blue-600">Manajemen Ruangan</a>
        <span>/</span>
        <span><?php echo e(isset($ruangan) ? 'Edit Ruangan & Isi' : 'Tambah Ruangan & Isi'); ?></span>
    </div>

    <h2 class="mb-8 text-2xl font-bold text-slate-900">
        <i class="fas fa-door-open text-blue-600 mr-2"></i>
        <?php echo e(isset($ruangan) ? 'Edit Ruangan & Isi Barang' : 'Tambah Ruangan & Isi Barang'); ?>
    ...
    <form action="<?php echo e(isset($ruangan) ? route('admin.ruangan.edit', $ruangan->id) : route('admin.ruangan.tambah')); ?>" method="POST" class="max-w-5xl space-y-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" id="kode_room" name="kode_room" value="<?php echo e($ruangan->kode_room ?? old('kode_room')); ?>" 
            ...
            <div>
                ...
                <input type="text" id="nama" name="nama" value="<?php echo e($ruangan->nama ?? old('nama')); ?>" 
                    class="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" 
            ...
            <div>
                ...
                <input type="text" id="lantai" name="lantai" value="<?php echo e($ruangan->lantai ?? old('lantai')); ?>"
                    class="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    ...
                    <option value="Tersedia" <?php echo e(($ruangan->status ?? old('status')) === 'Tersedia' ? 'selected' : ''); ?>>Tersedia</option>
                    <option value="Dipakai" <?php echo e(($ruangan->status ?? old('status')) === 'Dipakai' ? 'selected' : ''); ?>>Dipakai</option>
                    <option value="Maintenance" <?php echo e(($ruangan->status ?? old('status')) === 'Maintenance' ? 'selected' : ''); ?>>Maintenance</option>
                    <option value="Tidak Tersedia" <?php echo e(($ruangan->status ?? old('status')) === 'Tidak Tersedia' ? 'selected' : ''); ?>>Tidak Tersedia</option>
                </select>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody id="barang-table" class="divide-y divide-slate-200">
                        <?php if(isset($ruangan) && $ruangan->contents->count() > 0): ?>
                            <?php $__currentLoopData = $ruangan->contents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $content): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-50">
                                    <td class="px-4 py-3">
                                        <select name="barang[]" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" onchange="updateKode(this)">
                                            <option value="">-- Pilih Barang --</option>
                                            <?php $__currentLoopData = $semua_barang; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $barang): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($barang->id); ?>" <?php echo e($content->thing_id === $barang->id ? 'selected' : ''); ?>><?php echo e($barang->nama); ?></option>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </select>
                                    </td>
                                    <td class="px-4 py-3">
                                        <input type="text" class="w-full rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-sm" readonly value="<?php echo e($content->thing->kode_thing); ?>">
                                    </td>
                                    <td class="px-4 py-3">
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold">
                <i class="fas fa-save mr-1"></i> <?php echo e(isset($ruangan) ? 'Perbarui' : 'Simpan'); ?>

            </button>
            <a href="<?php echo e(route('admin.ruangan')); ?>" class="rounded-lg border border-slate-300 px-6 py-2 font-semibold text-slate-700 transition hover:bg-slate-100">
    ...
    row.innerHTML = `
        <td class="px-4 py-3">
            <select name="barang[]" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" onchange="updateKode(this)">
                <option value="">-- Pilih Barang --</option>
                <?php $__currentLoopData = $semua_barang; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $barang): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($barang->id); ?>"><?php echo e($barang->nama); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </td>
        <td class="px-4 py-3">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Projects\Sentra\resources\views/admin/sarana/ruangan/form-with-contents.blade.php ENDPATH**/ ?>